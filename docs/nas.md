...
**推荐：管理台网页直接上传（不用碰 File Station / Synology Photos）**

管理台「照片」页照片库区块有 **📤 家庭共享上传** 入口：填名字 →
选照片（多选）或选整个文件夹（保留子目录结构）→ 上传。链路：

1. 照片落本机 `photos/Frame_<名字>/`（同名自动跳过）
2. 自动启发式分析（免费）→ 照片库立刻出现 `Frame_<名字>` 分类
3. 自动增量推回 NAS `/volume1/photo/Frame_<名字>/` 备份
   （NAS 仍是最终存储；下次 NAS→本机同步两者一致，不重传）

NAS 侧已有照片的同步链路（两种方式殊途同归，都是 Frame 文件夹进照片库）：

1. 用户把照片放进 `Frame_<用户名>/`（网页上传 / File Station 均可）
