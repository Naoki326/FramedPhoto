...
- 文件夹管理（新建 / 重命名 / 删除空文件夹）与上传由服务端 API 经 SSH/rsync
  反向推回 NAS（ADR-0007：NAS photo 是唯一共享目录与最终存储）
...
4. 显示后深度休眠：睡到下一个本地 ：01/:31（心跳响应的 server_time 校准墙钟，
   见 `wake_align.c`）；时钟从未校准过时按 `FRAMEDPHOTO_DEEP_SLEEP_MIN` 兑底
