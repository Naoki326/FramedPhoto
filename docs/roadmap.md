...
- **照片库单一共享目录（ADR-0007）**：NAS `/volume1/photo` 共用（不按人建夹），
  上传目标 = 当前选中的分类 tab（含根目录/未分类），文件夹可新建 / 重命名 /
  删除空文件夹（NAS 先行、镜像与评分随迁）
- 天气卡四风格 + 城市自定义、管理台 24h 时序编排、屏幕校准工具链已上线
- **色彩显示修复**：色域外亮色墨水对混色 + 浓彩偏置可调已上线（ADR-0004）
- **照片分类（#25）**：照片库按 photo 第一层文件夹分类，时段可绑定分类、选片严格限分类内（三级降级不出圈，ADR-0005 / 0006）、照片库 tab 化（ADR-0007 文件夹可管理）
