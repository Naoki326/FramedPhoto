...
- 服务端 190+ 单测全绿，固件编译通过（esp32s3，958KB）；launchd 服务运行在 :8010
- **NAS 首次全量同步已完成**（rsync over SSH，373 文件 / 约 3.4GB），**全量 AI 照片分析已完成**（392 张全部入库评分）
- 天气卡四风格 + 城市自定义、管理台 24h 时序编排、屏幕校准工具链已上线
- **色彩显示修复**：色域外亮色墨水对混色 + 浓彩偏置可调已上线（ADR-0004）
- **照片分类（#25）**：照片库按 NAS 第一层文件夹分类，时段可绑定分类、选片严格限分类内（三级降级不出圈）、照片库 tab 化（ADR-0005 / 0006）
