...
- **照片库已本地化（ADR-0007）**：NAS 同步链路全面退役，管理台上传照片到文件夹，
  文件夹可新建 / 重命名 / 删除空文件夹；原 NAS 同步的 3.4GB 存量照片已清空
- 天气卡四风格 + 城市自定义、管理台 24h 时序编排、屏幕校准工具链已上线
- **色彩显示修复**：色域外亮色墨水对混色 + 浓彩偏置可调已上线（ADR-0004）
- **照片分类（#25）**：照片库按第一层文件夹分类，时段可绑定分类、选片严格限分类内（三级降级不出圈，ADR-0005 / 0006）、照片库 tab 化（ADR-0007 本地管理）
