...
id 命中某前缀即分发到对应源；无命中由调用方兜底（内容库源，后续接入）。

生成源的 id 是内容指纹（前缀-渲染字节哈希前 10 位），仅供设备变更检测，
不是查找键：源一律服务「当前内容」，不按指纹寻址。源不负责 url/preview_url
的组装（那是路由层 helper 的事，见 ADR-0002）。
"""
...
def resolve(content_id: str) -> ContentSource | None:
    """有序前缀表查源：首个命中前缀的源；无命中返回 None（兜底到内容库）。"""
...
def _ordered_table() -> list[tuple[str, ContentSource]]:
    ...
    from app import daily, free_module, weather_card
    return [
