 ...
 *
 * 硬件事实（来自官方 ESP32 示例，配套板为 ESP32-S3）：
 *  - GDEB0709E01 由 2 颗 GDEP133C02 驱动 IC 拼接：CS0=右半屏、CS1=左半屏
 *  - SPI 8-bit 命令相位（trans.cmd），CS 由 GPIO 手动控制（无 D/C 引脚）
 *  - 像素格式 4bit/像素（1 字节 2 像素），高 nibble 为偶数像素
 *
 * 图像行布局（每行 600 字节 = 1200 像素 @ 2px/byte）：
 *    前 300B 送 IC0（屏幕右半），后 300B 送 IC1（屏幕左半）
 *    行内像素从右到左：字节 k 对应逻辑像素 (1198-2k, 1197-2k)
