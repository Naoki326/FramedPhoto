...
| **每日窗口** | 天气、月历日历卡、每日寄语与自定义图文模块按时段轮换 |
| **六色渲染** | 1200 × 1600 图片适配、E Ink Spectra 6 色板量化、抖动与真机校准 |
| **长续航设备** | ESP32-S3 WiFi 拉取内容，显示后深度休眠，唤醒对齐到每个整点的 ：01/:31（内容未变不刷屏） |
| **远程维护** | 双 OTA 分区、SHA-256 校验、失败回滚 |
| **私人照片库** | NAS `/volume1/photo` 单一共享目录：rsync 增量同步 + 管理台上传自动推回；文件夹（即分类）可新建 / 重命名 / 删除空文件夹 |
...
```text
00:00 – 08:00   日历卡片：一页当月月历 + 每日家庭寄语
08:00 – 10:00   天气
10:00 – 21:00   每日精选（历史上的今天）
...
FramedPhoto 的核心不是“显示天气”，而是让同一块屏幕在一天里拥有不同的内容。天气只是其中一个时段，另外三个时段分别负责**日子**（日历）、**回忆**（照片）和**创造**（自由模块）。

### 1. 每日精选：把历史上的今天挂在墙上

服务端会从照片库中匹配今天的月-日，结合 AI 回忆度评分挑选一张照片，自动生成一句文案并渲染到画面上。没有同日照片时，会降级到全局高分且近期未展示的照片；也可以在管理台手动精选（按钮「设为今日精选」）。

<p align="center">
  <img src="docs/design-preview/daily_photo_demo.jpg" alt="每日精选演示：真实照片 + AI 回忆文案叠加" width="300">
</p>

<p align="center"><sub>每日精选演示：全彩照片底部叠加 AI 回忆文案（设备上会再经 Spectra 6 六色量化）</sub></p>

### 2. 日历卡片：一页月历 + 每日家庭寄语

纯本地计算的经典挂历式月历：公历大字 + 农历小字（初一显月名）+ 节气绿字 / 节日红字，周日红、周六蓝，今天用淡黄底格加红圈标出；底部每天一句 LLM 生成的家庭寄语（失败自动降级内置语料）。无网络依赖，恒可用，跨 0 点自动换月换寄语。
...
默认是“天气 → 照片 → 自由模块”的三段式，但并不是写死的。管理台提供 24 小时时间轴，可以：

- 拖动白色边界调整开始时间；
- 双击任意位置切开一个时段；
- 点击时段切换为“天气 / 日历 / 照片 / 自由模块”；
...
[
  ...
  {"start": "08:00", "type": "weather"},
  {"start": "10:00", "type": "photo"},
...
每个时段最短 30 分钟，最后一段自动延续到 24:00。配置保存后即时生效，设备下一次轮询时就会按照新的内容计划工作。

天气卡片默认按**出口 IP 自动定位**城市；如果你希望固定显示某个城市的天气（例如相框放在上海宝山），在管理台「天气卡片风格」下方的城市输入框填写城市名并保存即可：

- 支持中文城市名（如 `上海宝山`、`宝山`、`北京`），保存时自动解析为和风城市 ID；
- 也可以直接填和风 location：城市 ID（如 `101020300`）、经纬度（如 `121.49,31.40`）或拼音；
- 保存后输入框下方会显示解析结果（如 `已定位：101020300`），天气卡片立即按该城市渲染，无需等待缓存；
- 留空则回退到 IP 自动定位；查不到的城市会提示改用城市 ID。

天气卡片有 **Apple 天气风 / 多彩卡片 / 杂志排版 / 经典微调** 四种风格，默认每日轮换，也可在管理台固定某一款。
...
6. 心跳响应携带服务器时间校准设备墙钟，默认睡到下一个本地半点边界加一分钟
   （每个整点的 ：01 / :31）再 RTC 唤醒，唤醒时刻可预期；只有内容发生变化才刷新
   （避免无意义的屏幕刷新与耗电），随后继续深度休眠。
...
最小配置只需要启动服务端即可。AI、天气和自由模块都可以在后续按需开启，照片库同步需配置群晖 NAS（SSH），配置项和示例见 [`services/.env.example`](services/.env.example)。
...
idf.py menuconfig   # 配置 WiFi、服务端地址、深度休眠与唤醒对齐（默认对齐到整点 :01/:31）等
idf.py build
...
- [x] 日历卡片：一页月历（公历/农历/节气/节日，今日红圈，六色全用）+ LLM 每日家庭寄语
...
- [x] 照片库单一共享目录：NAS photo 共用（不按人建夹），上传到当前分类 tab（含根目录），文件夹新建 / 重命名 / 删除空文件夹（ADR-0007）
...
- 个人照片库（NAS 镜像）：`services/photos/`、`services/daily/`
- 上传文件、运行时数据库、日志和缓存
